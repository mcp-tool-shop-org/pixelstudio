...
Presets are persisted at `%LOCALAPPDATA%/PixelStudio/presets/{id}.preset.json` (user-level, not project-embedded). Overrides are transient — they only affect the current apply and never mutate saved preset defaults.
...
- **pixelstudio_native** (default): Rich manifest with export type, sheet dimensions, grid layout, generated timestamp, per-clip placements and files.
...
The catalog is stored at `{data_local_dir}/PixelStudio/asset-catalog.json`, separate from project files. It is an index layer — removing a catalog entry never deletes the backing `.pxs` file.
...
Thumbnails are 64×64 PNG images generated by nearest-neighbor downscale from the first composited frame. This preserves the crisp pixel-art look at small sizes. Thumbnails are stored at `{data_local_dir}/PixelStudio/thumbnails/{frame-id}.png` with deterministic paths based on the first frame's ID.
...
- **PixelStudio Native manifests** — full `package` object with `packageName`, `version`, `author`, `description`
